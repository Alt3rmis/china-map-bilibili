/**
 * 数据处理流水线
 * 从 raw_data.js 读取新数据，与现有 data.js 合并，保留 manualCity 值
 * 使用方法: node scripts/process-raw-data.js
 */

const fs = require('fs');
const path = require('path');

// 读取 raw_data.js
function readRawData() {
    const rawPath = path.join(__dirname, '..', 'data', 'raw_data.js');
    if (!fs.existsSync(rawPath)) {
        console.error('\n❌ 错误: data/raw_data.js 文件不存在');
        console.log('请先将 B站 控制台获取的数据粘贴到 data/raw_data.js 文件中');
        process.exit(1);
    }

    const rawContent = fs.readFileSync(rawPath, 'utf8');

    // 获取 RAW_DATA 数组
    const getData = new Function(rawContent + '; return typeof RAW_DATA !== "undefined" ? RAW_DATA : [];');
    const rawData = getData();

    if (!Array.isArray(rawData)) {
        console.error('\n❌ 错误: raw_data.js 中 RAW_DATA 不是数组');
        console.log('请检查 raw_data.js 格式，RAW_DATA 应该是数组');
        process.exit(1);
    }

    console.log(`✓ 读取原始数据：${rawData.length} 条视频`);
    return rawData;
}

// 读取现有 data.js（保留 manualCity）
function readExistingData() {
    const dataPath = path.join(__dirname, '..', 'data', 'data.js');

    if (!fs.existsSync(dataPath)) {
        console.log('⚠️  data.js 不存在，将创建新文件');
        return null;
    }

    const dataContent = fs.readFileSync(dataPath, 'utf8');

    // 使用 Function 构造器来获取导出的变量
    const getData = new Function(dataContent + '; return { provinceData: typeof provinceData !== "undefined" ? provinceData : null, cityData: typeof cityData !== "undefined" ? cityData : null };');
    const data = getData();

    if (data.provinceData) {
        console.log(`✓ 读取现有省份数据：${Object.keys(data.provinceData).length} 个省份`);
        return { type: 'province', data: data.provinceData };
    } else if (data.cityData) {
        console.log(`✓ 读取现有城市数据：${Object.keys(data.cityData).length} 个城市`);
        return { type: 'city', data: data.cityData };
    } else {
        console.log('⚠️  data.js 为空或格式错误');
        return null;
    }
}

// 按 title+url 匹配视频（用于保留 manualCity）
function findVideoByTitleAndUrl(existingData, newVideo) {
    const newVideoId = `${newVideo.title}-${newVideo.url}`;

    if (existingData.type === 'province') {
        // 在省份数据中查找
        for (const [province, provinceInfo] of Object.entries(existingData.data)) {
            const videos = provinceInfo.videos || [];
            for (const video of videos) {
                const existingId = `${video.title}-${video.url}`;
                if (existingId === newVideoId) {
                    return { found: true, video };
                }
            }
        }
    } else {
        // 在城市数据中查找
        for (const [city, cityInfo] of Object.entries(existingData.data)) {
            const videos = cityInfo.videos || [];
            for (const video of videos) {
                const existingId = `${video.title}-${video.url}`;
                if (existingId === newVideoId) {
                    return { found: true, video };
                }
            }
        }
    }

    return { found: false };
}

// 提取城市名称（使用 extract-cities.js 的逻辑）
function extractCitiesFromTitle(title) {
    // 完整的中国城市列表
    const ALL_CITIES = [
        // 直辖市
        '北京', '上海', '天津', '重庆',
        // 省会城市
        '广州', '深圳', '成都', '杭州', '南京', '武汉', '长沙', '西安', '郑州',
        '石家庄', '太原', '合肥', '南昌', '福州', '厦门', '南宁', '海口', '昆明', '贵阳',
        '拉萨', '兰州', '西宁', '银川', '乌鲁木齐', '呼和浩特', '长春', '沈阳', '哈尔滨', '济南',
        // 河北
        '唐山', '秦皇岛', '邯郸', '邢台', '保定', '张家口', '承德', '沧州', '廊坊', '衡水',
        '辛集', '晋州', '新乐', '遵化', '迁安', '武安', '南宫', '沙河', '涿州', '定州',
        '安国', '高碑店', '泊头', '任丘', '黄骅', '河间', '霸州', '三河', '深州',
        // 山西
        '大同', '阳泉', '长治', '晋城', '朔州', '晋中', '运城', '忻州', '临汾', '吕梁',
        '古交', '高平', '介休', '永济', '河津', '原平', '侯马', '霍州', '孝义', '汾阳',
        // 内蒙古
        '包头', '乌海', '赤峰', '通辽', '鄂尔多斯', '呼伦贝尔', '巴彦淖尔', '乌兰察布',
        '满洲里', '二连浩特', '乌兰浩特', '阿尔山', '锡林浩特', '巴彦浩特', '丰镇', '牙克石',
        // 辽宁
        '大连', '鞍山', '抚顺', '本溪', '丹东', '锦州', '营口', '阜新', '辽阳', '盘锦',
        '铁岭', '朝阳', '葫芦岛', '瓦房店', '海城', '东港', '凤城', '凌海', '北镇', '大石桥',
        '盖州', '灯塔', '调兵山', '开原',
        // 吉林
        '吉林', '四平', '辽源', '通化', '白山', '松原', '白城',
        '公主岭', '梅河口', '集安', '临江', '大安', '洮南', '珲春', '龙井', '和龙', '敦化',
        // 黑龙江
        '齐齐哈尔', '鸡西', '鹤岗', '双鸭山', '大庆', '伊春', '佳木斯', '七台河', '牡丹江',
        '黑河', '绥化', '绥芬河', '同江', '虎林', '密山', '富锦', '漠河', '铁力', '尚志', '五常', '海伦', '肇东',
        // 江苏
        '苏州', '无锡', '徐州', '常州', '南通', '连云港', '淮安', '盐城', '扬州', '镇江', '泰州', '宿迁',
        '江阴', '宜兴', '新沂', '邳州', '溧阳', '金坛', '常熟', '张家港', '昆山', '太仓', '启东', '如皋',
        // 浙江
        '宁波', '温州', '嘉兴', '湖州', '绍兴', '金华', '衢州', '舟山', '台州', '丽水',
        '建德', '临安', '余姚', '慈溪', '瑞安', '乐清', '海宁', '平湖', '桐乡', '诸暨', '嵊州',
        // 安徽
        '芜湖', '蚌埠', '淮南', '马鞍山', '淮北', '铜陵', '安庆', '黄山', '滁州', '阜阳', '宿州',
        '六安', '亳州', '池州', '宣城', '界首', '桐城', '宁国', '天长', '明光',
        // 福建
        '莆田', '三明', '泉州', '漳州', '南平', '龙岩', '宁德',
        '福清', '邵武', '武夷山', '建瓯', '永安', '石狮', '晋江', '南安', '龙海', '漳平',
        // 江西
        '景德镇', '萍乡', '九江', '新余', '鹰潭', '赣州', '吉安', '宜春', '抚州', '上饶',
        '瑞昌', '乐平', '瑞金', '德兴', '贵溪', '樟树', '丰城', '高安',
        // 山东
        '青岛', '淄博', '枣庄', '东营', '烟台', '潍坊', '济宁', '泰安', '威海', '日照', '临沂',
        '德州', '聊城', '滨州', '菏泽', '胶州', '即墨', '平度', '莱西', '滕州', '龙口',
        '莱阳', '莱州', '招远', '栖霞', '青州', '诸城', '寿光',
        // 河南
        '开封', '洛阳', '平顶山', '安阳', '鹤壁', '新乡', '焦作', '濮阳', '许昌', '漯河',
        '三门峡', '南阳', '商丘', '信阳', '周口', '驻马店', '荥阳', '新郑', '登封', '巩义',
        '新密', '偃师', '舞钢', '汝州', '林州', '卫辉', '辉县', '沁阳',
        // 湖北
        '黄石', '十堰', '宜昌', '襄阳', '鄂州', '荆门', '孝感', '荆州', '黄冈', '咸宁', '随州',
        '大冶', '丹江口', '宜都', '当阳', '枝江', '老河口', '枣阳', '宜城', '钟祥', '京山', '安陆', '潜江', '仙桃', '天门',
        // 湖南
        '株洲', '湘潭', '衡阳', '邵阳', '岳阳', '常德', '张家界', '益阳', '郴州', '永州',
        '怀化', '娄底', '浏阳', '醴陵', '韶山', '耒阳', '常宁', '武冈', '汨罗', '临湘',
        '津市', '沅江', '资兴', '洪江', '平江县',
        // 广东
        '珠海', '汕头', '佛山', '韶关', '湛江', '肇庆', '江门', '茂名', '惠州', '梅州',
        '汕尾', '河源', '阳江', '清远', '东莞', '中山', '潮州', '揭阳', '云浮',
        '乐昌', '南雄', '台山', '开平', '鹤山', '恩平', '廉江', '雷州', '吴川', '化州', '信宜', '高州',
        '四会', '兴宁', '陆丰', '阳春', '英德', '连州', '普宁', '罗定',
        // 广西
        '柳州', '桂林', '梧州', '北海', '防城港', '钦州', '贵港', '玉林', '百色', '贺州',
        '河池', '来宾', '崇左', '岑溪', '东兴', '桂平', '北流', '靖西', '平果', '凭祥', '合山', '荔浦', '横州',
        // 海南
        '三亚', '三沙', '儋州', '五指山', '琼海', '文昌', '万宁', '东方', '陵水',
        // 四川
        '自贡', '攀枝花', '泸州', '德阳', '绵阳', '广元', '遂宁', '内江', '乐山', '南充', '眉山', '宜宾', '广安', '达州', '雅安', '巴中', '资阳',
        '都江堰', '彭州', '邛崃', '崇州', '广汉', '什邡', '绵竹', '江油', '峨眉山', '阆中', '华蓥', '万源', '简阳', '西昌',
        // 贵州
        '六盘水', '遵义', '安顺', '毕节', '铜仁',
        '清镇', '赤水', '仁怀', '凯里', '都匀', '兴义', '福泉',
        // 云南
        '曲靖', '玉溪', '保山', '昭通', '丽江', '普洱', '临沧',
        '安宁', '宣威', '腾冲', '楚雄', '大理', '景洪', '芒市', '瑞丽',
        // 西藏
        '日喀则', '昌都', '林芝', '山南', '那曲',
        // 陕西
        '铜川', '宝鸡', '咸阳', '渭南', '延安', '汉中', '榆林', '安康', '商洛',
        '兴平', '韩城', '华阴',
        // 甘肃
        '金昌', '白银', '天水', '武威', '张掖', '平凉', '酒泉', '庆阳', '定西', '陇南',
        '玉门', '敦煌', '合作', '洮南市',
        // 青海
        '海东',
        '格尔木', '德令哈',
        // 宁夏
        '石嘴山', '吴忠', '固原', '中卫',
        '灵武', '青铜峡',
        // 新疆
        '克拉玛依', '吐鲁番', '哈密', '昌吉', '博尔塔拉', '巴音郭楞', '阿克苏', '克孜勒苏', '喀什', '和田', '伊犁', '塔城', '阿勒泰',
        '阜康', '库尔勒', '阿图什', '阿克苏市', '喀什市', '和田市', '伊宁', '奎屯', '塔城', '乌苏', '阿勒泰'
    ];

    // 按城市名长度降序排序，优先匹配更长的城市名
    ALL_CITIES.sort((a, b) => b.length - a.length);

    const foundCities = new Set();
    const checked = new Set();

    for (const city of ALL_CITIES) {
        const cityKey = city;
        if (checked.has(cityKey)) continue;
        checked.add(cityKey);

        // 使用正则匹配整个城市名（避免部分匹配）
        const regex = new RegExp(city, 'g');
        let match;
        while ((match = regex.exec(title)) !== null) {
            // 检查匹配是否完整
            const matchedText = match[0];
            if (matchedText === city) {
                foundCities.add(city);
            }
        }
    }

    return Array.from(foundCities).sort();
}

// 城市到省份的映射
const CITY_TO_PROVINCE = {
    // 直辖市
    '北京': '北京', '上海': '上海', '天津': '天津', '重庆': '重庆',
    // 省会城市
    '广州': '广东', '深圳': '广东', '成都': '四川', '杭州': '浙江', '南京': '江苏',
    '武汉': '湖北', '长沙': '湖南', '西安': '陕西', '郑州': '河南',
    '石家庄': '河北', '太原': '山西', '合肥': '安徽', '南昌': '江西',
    '福州': '福建', '厦门': '福建', '南宁': '广西', '海口': '海南',
    '昆明': '云南', '贵阳': '贵州', '拉萨': '西藏', '兰州': '甘肃',
    '西宁': '青海', '银川': '宁夏', '乌鲁木齐': '新疆', '呼和浩特': '内蒙古',
    '长春': '吉林', '沈阳': '辽宁', '哈尔滨': '黑龙江', '济南': '山东',
    // 河北
    '唐山': '河北', '秦皇岛': '河北', '邯郸': '河北', '邢台': '河北', '保定': '河北',
    '张家口': '河北', '承德': '河北', '沧州': '河北', '廊坊': '河北', '衡水': '河北',
    '辛集': '河北', '晋州': '河北', '新乐': '河北', '遵化': '河北', '迁安': '河北',
    '武安': '河北', '南宫': '河北', '沙河': '河北', '涿州': '河北', '定州': '河北',
    '安国': '河北', '高碑店': '河北', '泊头': '河北', '任丘': '河北', '黄骅': '河北',
    '河间': '河北', '霸州': '河北', '三河': '河北', '深州': '河北',
    // 山西
    '大同': '山西', '阳泉': '山西', '长治': '山西', '晋城': '山西', '朔州': '山西',
    '晋中': '山西', '运城': '山西', '忻州': '山西', '临汾': '山西', '吕梁': '山西',
    '古交': '山西', '高平': '山西', '介休': '山西', '永济': '山西', '河津': '山西',
    '原平': '山西', '侯马': '山西', '霍州': '山西', '孝义': '山西', '汾阳': '山西',
    // 内蒙古
    '包头': '内蒙古', '乌海': '内蒙古', '赤峰': '内蒙古', '通辽': '内蒙古',
    '鄂尔多斯': '内蒙古', '呼伦贝尔': '内蒙古', '巴彦淖尔': '内蒙古', '乌兰察布': '内蒙古',
    '满洲里': '内蒙古', '二连浩特': '内蒙古', '乌兰浩特': '内蒙古', '阿尔山': '内蒙古',
    '锡林浩特': '内蒙古', '巴彦浩特': '内蒙古', '丰镇': '内蒙古', '牙克石': '内蒙古',
    // 辽宁
    '大连': '辽宁', '鞍山': '辽宁', '抚顺': '辽宁', '本溪': '辽宁', '丹东': '辽宁',
    '锦州': '辽宁', '营口': '辽宁', '阜新': '辽宁', '辽阳': '辽宁', '盘锦': '辽宁',
    '铁岭': '辽宁', '朝阳': '辽宁', '葫芦岛': '辽宁', '瓦房店': '辽宁', '海城': '辽宁',
    '东港': '辽宁', '凤城': '辽宁', '凌海': '辽宁', '北镇': '辽宁', '大石桥': '辽宁',
    '盖州': '辽宁', '灯塔': '辽宁', '调兵山': '辽宁', '开原': '辽宁',
    // 吉林
    '吉林': '吉林', '四平': '吉林', '辽源': '吉林', '通化': '吉林', '白山': '吉林',
    '松原': '吉林', '白城': '吉林', '公主岭': '吉林', '梅河口': '吉林', '集安': '吉林',
    '临江': '吉林', '大安': '吉林', '洮南': '吉林', '珲春': '吉林', '龙井': '吉林', '和龙': '吉林', '敦化': '吉林',
    // 黑龙江
    '齐齐哈尔': '黑龙江', '鸡西': '黑龙江', '鹤岗': '黑龙江', '双鸭山': '黑龙江',
    '大庆': '黑龙江', '伊春': '黑龙江', '佳木斯': '黑龙江', '七台河': '黑龙江', '牡丹江': '黑龙江',
    '黑河': '黑龙江', '绥化': '黑龙江', '绥芬河': '黑龙江', '同江': '黑龙江', '虎林': '黑龙江',
    '密山': '黑龙江', '富锦': '黑龙江', '漠河': '黑龙江', '铁力': '黑龙江', '尚志': '黑龙江',
    '五常': '黑龙江', '海伦': '黑龙江', '肇东': '黑龙江',
    // 江苏
    '苏州': '江苏', '无锡': '江苏', '徐州': '江苏', '常州': '江苏', '南通': '江苏', '连云港': '江苏',
    '淮安': '江苏', '盐城': '江苏', '扬州': '江苏', '镇江': '江苏', '泰州': '江苏', '宿迁': '江苏',
    '江阴': '江苏', '宜兴': '江苏', '新沂': '江苏', '邳州': '江苏', '溧阳': '江苏', '金坛': '江苏',
    '常熟': '江苏', '张家港': '江苏', '昆山': '江苏', '太仓': '江苏', '启东': '江苏', '如皋': '江苏',
    // 浙江
    '宁波': '浙江', '温州': '浙江', '嘉兴': '浙江', '湖州': '浙江', '绍兴': '浙江',
    '金华': '浙江', '衢州': '浙江', '舟山': '浙江', '台州': '浙江', '丽水': '浙江',
    '建德': '浙江', '临安': '浙江', '余姚': '浙江', '慈溪': '浙江', '瑞安': '浙江',
    '乐清': '浙江', '海宁': '浙江', '平湖': '浙江', '桐乡': '浙江', '诸暨': '浙江', '嵊州': '浙江',
    // 安徽
    '芜湖': '安徽', '蚌埠': '安徽', '淮南': '安徽', '马鞍山': '安徽', '淮北': '安徽',
    '铜陵': '安徽', '安庆': '安徽', '黄山': '安徽', '滁州': '安徽', '阜阳': '安徽', '宿州': '安徽',
    '六安': '安徽', '亳州': '安徽', '池州': '安徽', '宣城': '安徽', '界首': '安徽', '桐城': '安徽',
    '宁国': '安徽', '天长': '安徽', '明光': '安徽',
    // 福建
    '莆田': '福建', '三明': '福建', '泉州': '福建', '漳州': '福建', '南平': '福建',
    '龙岩': '福建', '宁德': '福建', '福清': '福建', '邵武': '福建', '武夷山': '福建',
    '建瓯': '福建', '永安': '福建', '石狮': '福建', '晋江': '福建', '南安': '福建',
    '龙海': '福建', '漳平': '福建',
    // 江西
    '景德镇': '江西', '萍乡': '江西', '九江': '江西', '新余': '江西', '鹰潭': '江西',
    '赣州': '江西', '吉安': '江西', '宜春': '江西', '抚州': '江西', '上饶': '江西',
    '瑞昌': '江西', '乐平': '江西', '瑞金': '江西', '德兴': '江西', '贵溪': '江西',
    '樟树': '江西', '丰城': '江西', '高安': '江西',
    // 山东
    '青岛': '山东', '淄博': '山东', '枣庄': '山东', '东营': '山东', '烟台': '山东',
    '潍坊': '山东', '济宁': '山东', '泰安': '山东', '威海': '山东', '日照': '山东', '临沂': '山东',
    '德州': '山东', '聊城': '山东', '滨州': '山东', '菏泽': '山东', '胶州': '山东',
    '即墨': '山东', '平度': '山东', '莱西': '山东', '滕州': '山东', '龙口': '山东',
    '莱阳': '山东', '莱州': '山东', '招远': '山东', '栖霞': '山东', '青州': '山东', '诸城': '山东', '寿光': '山东',
    // 河南
    '开封': '河南', '洛阳': '河南', '平顶山': '河南', '安阳': '河南', '鹤壁': '河南',
    '新乡': '河南', '焦作': '河南', '濮阳': '河南', '许昌': '河南', '漯河': '河南',
    '三门峡': '河南', '南阳': '河南', '商丘': '河南', '信阳': '河南', '周口': '河南',
    '驻马店': '河南', '荥阳': '河南', '新郑': '河南', '登封': '河南', '巩义': '河南',
    '新密': '河南', '偃师': '河南', '舞钢': '河南', '汝州': '河南', '林州': '河南',
    '卫辉': '河南', '辉县': '河南', '沁阳': '河南',
    // 湖北
    '黄石': '湖北', '十堰': '湖北', '宜昌': '湖北', '襄阳': '湖北', '鄂州': '湖北',
    '荆门': '湖北', '孝感': '湖北', '荆州': '湖北', '黄冈': '湖北', '咸宁': '湖北',
    '随州': '湖北', '大冶': '湖北', '丹江口': '湖北', '宜都': '湖北', '当阳': '湖北',
    '枝江': '湖北', '老河口': '湖北', '枣阳': '湖北', '宜城': '湖北', '钟祥': '湖北',
    '京山': '湖北', '安陆': '湖北', '潜江': '湖北', '仙桃': '湖北', '天门': '湖北',
    // 湖南
    '株洲': '湖南', '湘潭': '湖南', '衡阳': '湖南', '邵阳': '湖南', '岳阳': '湖南',
    '常德': '湖南', '张家界': '湖南', '益阳': '湖南', '郴州': '湖南', '永州': '湖南',
    '怀化': '湖南', '娄底': '湖南', '浏阳': '湖南', '醴陵': '湖南', '韶山': '湖南',
    '耒阳': '湖南', '常宁': '湖南', '武冈': '湖南', '汨罗': '湖南', '临湘': '湖南',
    '津市': '湖南', '沅江': '湖南', '资兴': '湖南', '洪江': '湖南', '平江县': '湖南',
    // 广东
    '珠海': '广东', '汕头': '广东', '佛山': '广东', '韶关': '广东', '湛江': '广东',
    '肇庆': '广东', '江门': '广东', '茂名': '广东', '惠州': '广东', '梅州': '广东',
    '汕尾': '广东', '河源': '广东', '阳江': '广东', '清远': '广东', '东莞': '广东',
    '中山': '广东', '潮州': '广东', '揭阳': '广东', '云浮': '广东', '乐昌': '广东',
    '南雄': '广东', '台山': '广东', '开平': '广东', '鹤山': '广东', '恩平': '广东', '廉江': '广东',
    '雷州': '广东', '吴川': '广东', '化州': '广东', '信宜': '广东', '高州': '广东',
    '四会': '广东', '兴宁': '广东', '陆丰': '广东', '阳春': '广东', '英德': '广东',
    '连州': '广东', '普宁': '广东', '罗定': '广东',
    // 广西
    '柳州': '广西', '桂林': '广西', '梧州': '广西', '北海': '广西', '防城港': '广西',
    '钦州': '广西', '贵港': '广西', '玉林': '广西', '百色': '广西', '贺州': '广西',
    '河池': '广西', '来宾': '广西', '崇左': '广西', '岑溪': '广西', '东兴': '广西',
    '桂平': '广西', '北流': '广西', '靖西': '广西', '平果': '广西', '凭祥': '广西',
    '合山': '广西', '荔浦': '广西', '横州': '广西',
    // 海南
    '三亚': '海南', '三沙': '海南', '儋州': '海南', '五指山': '海南', '琼海': '海南',
    '文昌': '海南', '万宁': '海南', '东方': '海南', '陵水': '海南',
    // 四川
    '自贡': '四川', '攀枝花': '四川', '泸州': '四川', '德阳': '四川', '绵阳': '四川',
    '广元': '四川', '遂宁': '四川', '内江': '四川', '乐山': '四川', '南充': '四川',
    '眉山': '四川', '宜宾': '四川', '广安': '四川', '达州': '四川', '雅安': '四川',
    '巴中': '四川', '资阳': '四川', '都江堰': '四川', '彭州': '四川', '邛崃': '四川',
    '崇州': '四川', '广汉': '四川', '什邡': '四川', '绵竹': '四川', '江油': '四川',
    '峨眉山': '四川', '阆中': '四川', '华蓥': '四川', '万源': '四川', '简阳': '四川', '西昌': '四川',
    // 贵州
    '六盘水': '贵州', '遵义': '贵州', '安顺': '贵州', '毕节': '贵州', '铜仁': '贵州',
    '清镇': '贵州', '赤水': '贵州', '仁怀': '贵州', '凯里': '贵州', '都匀': '贵州',
    '兴义': '贵州', '福泉': '贵州',
    // 云南
    '曲靖': '云南', '玉溪': '云南', '保山': '云南', '昭通': '云南', '丽江': '云南',
    '普洱': '云南', '临沧': '云南', '安宁': '云南', '宣威': '云南', '腾冲': '云南',
    '楚雄': '云南', '大理': '云南', '景洪': '云南', '芒市': '云南', '瑞丽': '云南',
    // 西藏
    '日喀则': '西藏', '昌都': '西藏', '林芝': '西藏', '山南': '西藏', '那曲': '西藏',
    // 陕西
    '铜川': '陕西', '宝鸡': '陕西', '咸阳': '陕西', '渭南': '陕西', '延安': '陕西',
    '汉中': '陕西', '榆林': '陕西', '安康': '陕西', '商洛': '陕西', '兴平': '陕西',
    '韩城': '陕西', '华阴': '陕西',
    // 甘肃
    '金昌': '甘肃', '白银': '甘肃', '天水': '甘肃', '武威': '甘肃', '张掖': '甘肃',
    '平凉': '甘肃', '酒泉': '甘肃', '庆阳': '甘肃', '定西': '甘肃', '陇南': '甘肃',
    '玉门': '甘肃', '敦煌': '甘肃', '合作': '甘肃', '洮南市': '甘肃',
    // 青海
    '海东': '青海', '格尔木': '青海', '德令哈': '青海',
    // 宁夏
    '石嘴山': '宁夏', '吴忠': '宁夏', '固原': '宁夏', '中卫': '宁夏',
    '灵武': '宁夏', '青铜峡': '宁夏',
    // 新疆
    '克拉玛依': '新疆', '吐鲁番': '新疆', '哈密': '新疆', '昌吉': '新疆', '博尔塔拉': '新疆',
    '巴音郭楞': '新疆', '阿克苏': '新疆', '克孜勒苏': '新疆', '喀什': '新疆', '和田': '新疆',
    '伊犁': '新疆', '塔城': '新疆', '阿勒泰': '新疆', '阜康': '新疆', '库尔勒': '新疆',
    '阿图什': '新疆', '阿克苏市': '新疆', '喀什市': '新疆', '和田市': '新疆', '伊宁': '新疆',
    '奎屯': '新疆', '塔城': '新疆', '乌苏': '新疆', '阿勒泰': '新疆'
};

// 智能选择城市：优先选择省份后面的城市（避免误选省份名）
function selectBestCity(autoCities, title) {
    if (autoCities.length === 0) {
        return null;
    }

    if (autoCities.length === 1) {
        return autoCities[0];
    }

    // 查找标题中的"快递里的中国"标记后的城市
    const markerIndex = title.indexOf('快递里的中国');
    if (markerIndex === -1) {
        // 没有找到标记，返回第一个城市
        return autoCities[0];
    }

    // 提取标记后的文本
    const afterMarker = title.substring(markerIndex);

    // 过滤出在标记后出现的城市
    const citiesAfterMarker = autoCities.filter(city => {
        // 查找城市在标记后文本中的最后位置
        const lastIndex = afterMarker.lastIndexOf(city);
        if (lastIndex === -1) {
            return false;
        }

        // 确保城市名是完整匹配（不是子串）
        const before = afterMarker[lastIndex - 1] || '';
        const after = afterMarker[lastIndex + city.length] || '';
        const hasWordBoundary = !isWordChar(before) && !isWordChar(after);

        return hasWordBoundary;
    });

    // 如果标记后找到了城市
    if (citiesAfterMarker.length > 0) {
        // 如果只有一个城市，直接返回
        if (citiesAfterMarker.length === 1) {
            return citiesAfterMarker[0];
        }

        // 如果有多个城市（例如："吉林 白城"），优先选择非省份名的城市
        // 获取所有城市对应的省份
        const citiesWithProvinces = citiesAfterMarker.map(city => ({
            city,
            province: CITY_TO_PROVINCE[city]
        }));

        // 检查是否有省份名重复的情况（即城市名也是省份名）
        const hasProvinceNameAsCity = citiesWithProvinces.some(c => c.city === c.province);

        if (hasProvinceNameAsCity) {
            // 找出不是省份名的城市
            const nonProvinceCities = citiesAfterMarker.filter(city => {
                const province = CITY_TO_PROVINCE[city];
                return province && city !== province;
            });

            // 如果找到了非省份名的城市，返回第一个
            if (nonProvinceCities.length > 0) {
                return nonProvinceCities[0];
            }
        }

        // 回退：返回第一个城市
        return citiesAfterMarker[0];
    }

    // 回退：返回按长度排序后的第一个城市
    return autoCities[0];
}

// 判断是否是文字字符（用于单词边界检查）
function isWordChar(char) {
    return /[\u4e00-\u9fa5a-zA-Z0-9]/.test(char);
}

// 将原始数据转换为城市格式（保留 manualCity）
function processRawData(rawData, existingData) {
    const cityData = {};

    rawData.forEach(rawVideo => {
        // 提取城市名称
        const autoCities = extractCitiesFromTitle(rawVideo.title);

        // 智能选择最佳城市
        let city = selectBestCity(autoCities, rawVideo.title);
        let manualCity = null;

        // 查找是否存在相同的视频（保留 manualCity）
        const existing = findVideoByTitleAndUrl(existingData, rawVideo);

        if (existing.found) {
            // 只保留 manualCity，城市名称使用新计算的值
            manualCity = existing.video.manualCity;

            // 如果有 manualCity，优先使用
            if (manualCity) {
                city = manualCity;
                console.log(`  ✓ 使用 manualCity: ${city}`);
            }
        }

        if (!city) {
            console.warn(`  ⚠️  无法提取城市: ${rawVideo.title}`);
            return;
        }

        // 获取省份
        const province = CITY_TO_PROVINCE[city];

        if (!province) {
            console.warn(`  ⚠️  无法确定城市 "${city}" 的省份`);
            return;
        }

        // 添加到城市数据
        if (!cityData[city]) {
            cityData[city] = { videos: [] };
        }

        cityData[city].videos.push({
            title: rawVideo.title,
            url: rawVideo.url,
            date: rawVideo.date || new Date().toISOString().split('T')[0],
            aid: rawVideo.aid || 0,
            bvid: rawVideo.bvid || '',
            views: rawVideo.view || rawVideo.views || 0,
            danmaku: rawVideo.danmaku || 0,
            city: city,
            autoCities: autoCities,
            manualCity: manualCity  // 保留现有 manualCity
        });
    });

    return cityData;
}

// 转换为省份数据
function convertToProvinceData(cityData) {
    const provinceData = {};

    Object.entries(cityData).forEach(([city, cityInfo]) => {
        const videos = cityInfo.videos || [];

        videos.forEach(video => {
            const city = video.city || '未分类';
            const province = CITY_TO_PROVINCE[city];

            if (!province) {
                console.warn(`⚠️  无法确定城市 "${city}" 的省份`);
                return;
            }

            if (!provinceData[province]) {
                provinceData[province] = { videos: [] };
            }

            provinceData[province].videos.push(video);
        });
    });

    return provinceData;
}

// 生成输出
function generateOutput(provinceData) {
    const sortedProvinces = Object.keys(provinceData).sort();

    const provincesStr = sortedProvinces.map(province => {
        const provinceVideos = provinceData[province].videos.sort((a, b) => new Date(b.date) - new Date(a.date));

        const videosStr = provinceVideos.map(video => {
            const title = video.title.replace(/\\/g, '\\\\').replace(/"/g, '\\"');
            const autoCitiesStr = JSON.stringify(video.autoCities);
            const manualCityStr = video.manualCity ? `"${video.manualCity}"` : 'null';
            const cityStr = video.city ? `"${video.city}"` : 'null';

            return `        {
                title: "${title}",
                url: "${video.url}",
                date: "${video.date}",
                aid: ${video.aid},
                bvid: "${video.bvid}",
                views: ${video.views},
                danmaku: ${video.danmaku},
                city: ${cityStr},
                autoCities: ${autoCitiesStr},
                manualCity: ${manualCityStr}
            }`;
        }).join(',\n');

        return `    "${province}": {
        videos: [
${videosStr}
        ]
    }`;
    }).join(',\n\n');

    return `// 城市数据配置（按省份组织）
// 城市名称需要与 ECharts 地图数据匹配
// 数据来源：哔哩哔哩UP主 老猫鱼不吃鱼
// 最后更新时间: ${new Date().toISOString()}
//
// 数据说明：
// - city: 城市名称（优先使用 manualCity，否则使用 autoCities）
// - autoCities: 自动从标题中提取的城市数组
// - manualCity: 人工手动标记的城市
//
// 省份包含该省份下的所有城市视频

const provinceData = {
${provincesStr}
};
`;
}

// 主函数
async function main() {
    console.log('='.repeat(60));
    console.log('数据处理流水线');
    console.log('='.repeat(60));

    // 读取原始数据
    const rawData = readRawData();

    // 读取现有数据（保留 manualCity）
    const existingData = readExistingData();

    // 处理数据
    console.log('\n正在处理数据...');
    const cityData = processRawData(rawData, existingData);

    if (Object.keys(cityData).length === 0) {
        console.error('\n❌ 错误: 没有有效的数据');
        process.exit(1);
    }

    // 转换为省份数据
    const provinceData = convertToProvinceData(cityData);

    // 生成输出
    const output = generateOutput(provinceData);

    // 统计信息
    console.log('\n' + '='.repeat(60));
    console.log('📊 处理统计');
    console.log('='.repeat(60));
    console.log(`\n总视频数: ${Object.values(provinceData).reduce((sum, p) => sum + p.videos.length, 0)}`);
    console.log(`覆盖省份数: ${sortedProvinces = Object.keys(provinceData).sort().length}`);
    console.log(`覆盖城市数: ${Object.keys(cityData).length}`);

    // 显示保留的 manualCity
    let preservedManualCities = 0;
    Object.values(provinceData).forEach(p => {
        p.videos.forEach(v => {
            if (v.manualCity) {
                preservedManualCities++;
            }
        });
    });
    if (preservedManualCities > 0) {
        console.log(`\n✓ 保留现有 manualCity: ${preservedManualCities} 个`);
    }

    // 写入文件
    const outputPath = path.join(__dirname, '..', 'data', 'data.js');
    fs.writeFileSync(outputPath, output, 'utf8');

    console.log('\n' + '='.repeat(60));
    console.log('✓ 数据已处理并保存到: data/data.js');
    console.log('='.repeat(60));
}

// 运行主函数
main().catch(error => {
    console.error('\n❌ 发生错误:', error);
    process.exit(1);
});
